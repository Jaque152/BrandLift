...
import { usePathname, useRouter } from "next/navigation"; // 1. Importamos hooks de navegación
...
export function Header({ dict, lang }: HeaderProps) {
  ...
  // 2. Inicializamos router y pathname
  ...
  const navLinks = [
    { key: "home", label: dict.home },
    { key: "about", label: dict.about },
    { key: "services", label: dict.services },
    { key: "contact", label: dict.contact },
  ];
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-cream-100/90 backdrop-blur-lg shadow-sm py-4" : "bg-transparent py-6"}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {/* 4. Botón Toggle de Idioma */}
          <Button 
            onClick={toggleLanguage} 
            variant="ghost" 
            className="text-charcoal-700 hover:text-terracotta-500 hover:bg-cream-200 transition-colors font-semibold"
          >
            {lang === 'en' ? 'ES' : 'EN'}
          </Button>
      ...
      <div className={`md:hidden absolute top-full left-0 right-0 bg-cream-100/95 backdrop-blur-lg border-b border-cream-300 overflow-hidden transition-all duration-500 ${mobileMenuOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
        <nav className="flex flex-col items-center gap-4 py-8">
          {navLinks.map((item) => (
            <a key={item.key} href={`#${item.key}`} onClick={() => setMobileMenuOpen(false)} className="text-charcoal-700 hover:text-terracotta-500 transition-colors duration-300 text-lg font-medium tracking-wide uppercase">{item.label}</a>
          ))}
