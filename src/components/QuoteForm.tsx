...
export function QuoteForm({ isOpen, onClose, dict, lang }: QuoteFormProps) {
  ...
  const { items, subtotal, ivaAmount, totalWithIva, clearCart, addCustomItem } = useCart();
    ...
    try {
      await fetch("/api/send-email", { 
        ...
        body: JSON.stringify({
          ...
          // Enviamos arrays y totales vacíos/en cero ya que es solo contacto
          items: [], 
          subtotal: 0,
          iva: 0,
          total: 0
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl bg-cream-50 border-cream-300 max-h-[90vh] overflow-hidden flex flex-col">
        {isSuccess ? (
          ...
          <>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 min-h-0">
              ...
              <TabsContent value="existing" className="mt-4">
                <form onSubmit={handleSubmitExisting} className="flex flex-col gap-6">
              ...
              <TabsContent value="new" className="mt-4">
                <form onSubmit={handleSubmitNew} className="flex flex-col gap-4">
                  ...
                  {/* ZONA SCROLLABLE SOLO PARA LOS INPUTS */}
                  <ScrollArea className="max-h-[45vh] pr-4">
                    <div className="space-y-4 pb-2">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="phone" className="text-charcoal-700">{dict?.phone || "Teléfono"}</Label>
                  ...
                  {/* BOTONES FIJOS ABAJO (Fuera del ScrollArea) */}
                  <div className="flex gap-3 pt-4 border-t border-cream-200">
                    <Button type="button" variant="outline" onClick={handleClose} className="flex-1 border-charcoal-300 text-charcoal-600 hover:bg-cream-200">{dict?.btnCancel || "Cancelar"}</Button>
